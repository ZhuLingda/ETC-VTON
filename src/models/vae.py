...
from .UpDecoder2D_Skip_Attn import get_up_block
...
class Decoder(nn.Module):
    def __init__(
        ...
        cross_attention_dims = reversed_block_out_channels[1:]
        cross_attention_dims.append(cross_attention_dims[-1])
        output_channel = reversed_block_out_channels[0]
        # hard-written for skip attention blocks
        attn_heads_list = [8, 8, 8, 4]
        attn_dim_head_list = [64, 64, 32, 32]

        for i, up_block_type in enumerate(up_block_types):
            prev_output_channel = output_channel
            output_channel = reversed_block_out_channels[i]
            cross_attention_dim = cross_attention_dims[i]
            heads = attn_heads_list[i]
            dim_head = attn_dim_head_list[i]
    ...
    def forward(self, z, intermediate_features=None, int_layers=None):
        ...
        if intermediate_features:
            ...
            for up_block, int_feat in zip(self.up_blocks, intermediate_features):
                ...
                sample = up_block(sample, int_feat)
        else:
            # up
